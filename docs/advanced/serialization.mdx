...
# Serialize to JSON (mode='json' handles datetime serialization)
messages_data = [msg.model_dump(mode='json') for msg in ctx.messages]
...
for msg in restored_ctx.messages:
...
# Save to file manually (mode='json' handles datetime)
ctx = get_context("main")
ctx.user("Hello")

with open("context.json", "w") as f:
    json.dump([msg.model_dump(mode='json') for msg in ctx.messages], f, indent=2)
