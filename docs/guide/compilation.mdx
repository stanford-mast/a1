...
Compile once, execute repeatedly with the same code. Best for consistency. The compiled code may include LLM calls if an LLM tools is supplied to the agent.
...
runtime = Runtime(generate=MyCodeGenerator(llm_tool=LLM("gpt-4.1")))
