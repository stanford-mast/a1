---
title: Compilation
description: AOT compilation and JIT execution modes
---

A1 compiles agents to Python code using two modes: AOT (compile once, run many times) or JIT (compile and run immediately for each unique input).

## AOT (Ahead-of-Time)

Compile once, execute repeatedly with the same code. Best for production workflows.
...
Generate and execute code immediately, optimized for each unique input.
