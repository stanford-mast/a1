...
RAG tools provide agents access to files, databases, and external knowledge. Tools are instantiated from DB connection strings or fsspec paths (local filesystem, S3, GCS, etc).
...
tools = rag.database("duckdb://path/to/db.duckdb")

# Use in agent
agent = Agent(
    name="db_agent",
    description="Queries database",
    tools=[*tools, LLM("gpt-4o")]
)
```
...
# Local filesystem
tools = rag.filesystem("/path/to/files")

# S3 bucket
tools = rag.filesystem("s3://my-bucket/path")
agent = Agent(
    name="research_agent",
