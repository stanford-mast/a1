...
from a1 import EM, LLM, Agent, Runtime

# Create a large enum (200 values)
LargeCategory = Enum("LargeCategory", {f"CATEGORY_{i:03d}": f"category_{i}" for i in range(200)})
...
async def test_small_enum_without_em_succeeds():
    ...
    small_category = Enum("SmallCategory", {f"CAT_{i:02d}": f"cat_{i}" for i in range(50)})

    class OutputWithSmallEnum(BaseModel):
        model_config = ConfigDict(extra="forbid")
        category: small_category = Field(description="Category")
        result: str = Field(description="Result")
