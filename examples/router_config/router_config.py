...
from a1 import EM, LLM, Agent, Done, Runtime, Strategy, Tool
...
def create_tool_from_json(cmd_name: str, cmd_config: dict) -> Tool:
    ...
    input_model = json_schema_to_pydantic(schema_name, schema_dict)

    # Create the execute function
    async def execute(**kwargs) -> CommandResult:
        """Dynamic tool function - validates and executes command"""
        return CommandResult(command=cmd_name, status="success", parameters=kwargs)

    # Create Tool instance
    return Tool(
        name=cmd_name, description=description, input_schema=input_model, output_schema=CommandResult, execute=execute
    )
...
async def main():
    ...
    try:
        result = await runtime.jit(agent, task="Create VLAN 100 named 'Engineering'")
        print("\n✅ Success!")
